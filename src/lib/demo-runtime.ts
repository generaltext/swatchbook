...
//
// It implements just the GtApi surface Swatchbook uses (see src/gt.d.ts), backed
// by plain strings in memory and mirrored to localStorage so edits survive a
// refresh. There's no CRDT and no network — it's single-user, local, throwaway
// sample data. `mode: 'demo'` (with `isLocal: true` as the back-compat signal)
// makes the data layer seed sample projects on first run; `version: 'demo'` lets
// the app show a "Demo" badge. Inert under a real host (we only install it when
// window.gt is absent).

const PREFIX = 'swatchbook-demo:'
